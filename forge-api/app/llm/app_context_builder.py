...
class AppContextBuilder:
    ...
    def _workflow_examples_section(self) -> str:
        return (
            ...
            "| Entity | ID | Create | Update | What it is |\n"
            "|--------|-----|--------|--------|------------|\n"
            "| Task | T-NNN | createTask | updateTask | Concrete work item (feature, bug, chore) |\n"
            "| Decision | D-NNN | createDecision | updateDecision | Why-choice: architecture, risk, exploration |\n"
            "| Objective | O-NNN | createObjective | updateObjective | Business goal with key results |\n"
            "| Idea | I-NNN | createIdea | updateIdea | Proposal/concept for future work |\n"
            "| Knowledge | K-NNN | createKnowledge | updateKnowledge | Domain rules, patterns, technical context |\n"
            "| Guideline | G-NNN | createGuideline | updateGuideline | Coding standard, convention, rule |\n"
            "| Lesson | L-NNN | createLesson | updateLesson | Pattern/insight from execution |\n"
            "| Research | R-NNN | createResearch | updateResearch | Structured analysis/findings |\n"
            "| AC Template | AC-NNN | createACTemplate | updateACTemplate | Reusable acceptance criteria |\n\n"
            "**All reads**: searchEntities, getEntity, listEntities (pass entity_type).\n"
            "**Archive**: archiveEntity (soft-delete with confirm=true).\n\n"
            "## CRUD Patterns\n\n"
            "**IMPORTANT: For update operations, call the update tool DIRECTLY with the new values. "
            "Do NOT read the entity first — update tools accept partial fields.**\n\n"
            "**Create** → use `create{Entity}(...)` with required fields.\n"
            "**Read** → `getEntity(entity_type, entity_id)` or `searchEntities(entity_type, query)`.\n"
            "**Update** → `update{Entity}(id, field=newValue)` — only pass changed fields.\n"
            "**Delete** → `archiveEntity(entity_type, entity_id, confirm=true)` — soft-delete.\n"
            "**List** → `listEntities(entity_type, filters)` — optional status/type filters.\n\n"
            "## Common Workflows\n\n"
            "**Find and view an entity:**\n"
            "`searchEntities(query, entity_type)` → `getEntity(entity_type, entity_id)`\n\n"
            "**Create a task with context:**\n"
            "`createTask(name='setup-database', description='Install and configure PostgreSQL', project='my-proj')`\n\n"
            "**Update a task:**\n"
            "`updateTask(task_id='T-001', description='Updated description')` — no need to getEntity first!\n\n"
            "**Record a decision:**\n"
            "`createDecision(task_id='T-001', type='architecture', issue='...', recommendation='...', project='my-proj')`\n\n"
            "**Close a decision:**\n"
            "`updateDecision(id='D-001', status='CLOSED', resolution_notes='...')` — no need to read first!\n\n"
            "**Update KR progress:**\n"
            "`updateObjective(id='O-001', key_results=[{id: 'KR-1', current: 150}])`\n\n"
