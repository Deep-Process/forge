...
export function SkillRow({ skill: s, selected, onSelect }: SkillRowProps) {
  ...
  return (
    <div className="flex items-center gap-3 px-3 py-2 rounded-md border bg-white hover:border-forge-300 transition-colors group">
      ...
      <Link
        href={`/skills/${s.id}`}
        className="text-xs text-gray-400 font-mono w-14 flex-shrink-0 hover:text-forge-600"
      >
        {s.id}
      </Link>
      ...
      <span className={`inline-flex items-center rounded-full px-2 py-0.5 text-[10px] font-medium ${cat.bg} ${cat.text} w-24 justify-center`}>
